...
switch ($queHago) {
    case "TraerTodos_usuarios":
        try {
            ...
            $objetos = $sql->prepare("SELECT * FROM usuarios");
          ...
          $table = "<table>
            ...
            <td>Estado</td>
             ...
             while ($row = $objetos->fetch(PDO::FETCH_LAZY)){ //fetch_all / fetch_assoc / fetch_array([MYSQLI_NUM | MYSQLI_ASSOC | MYSQLI_BOTH])
            $newElement ="
            <tr>
            <td>". $row->id ."</td>
            <td>". $row->nombre ."</td>
    ...
    case "TraerPorId_Usuarios":
        $objetos = $sql->prepare("SELECT `id`, `nombre`, `apellido`, `clave`, `perfil`, `estado` FROM `usuarios` WHERE `id`= ?");
        $objetos->bindParam(1, $idUsuario, PDO::PARAM_INT);
        $datos = $objetos->execute();          
        var_dump($datos);
        while ($row = $objetos->fetch(PDO::FETCH_LAZY)){ //fetch_all / fetch_assoc / fetch_array([MYSQLI_NUM | MYSQLI_ASSOC | MYSQLI_BOTH])
            $user_arr = $row;
            
           echo $user_arr.id . " Usuario: ". $user_arr.nombre;
        }     
